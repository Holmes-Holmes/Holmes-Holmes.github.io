...
class GroundTruth(object):
    ...
    def field_sample_choose(self):
        ...
        for id in source_field_dic.keys():
            flag = 0
            cve_flag = False
            if "github" in source_field_dic[id].keys() or  "gitlab" in source_field_dic[id].keys() or "snyk" in source_field_dic[id].keys() or "veracode" in source_field_dic[id].keys():
                flag = 1
    ...
    def entity_sample_dict(self, entity_have_pair_num, entity_have_pair_dic):
        ...
        print("3\% 10\% 15\% 20\% 30 \%  \samplenum {},{},{},{},{}".format(len(five_percent_sample_index_li),len(ten_percent_sample_index_li), len(fifteen_percent_sample_index_li),len(twenty_percent_sample_index_li), len(thirty_percent_sample_index_li)))
    ...
    def save_id_to_local(self, entity_have_pair_dic, diff_sample_index_li):
        ...
        percent_li = ["3%", "10%", "15%", "20%", "30%"]
        sample_dic  ={}
        holmes_id_li = list(entity_have_pair_dic.keys())
        for i in range(len(percent_li)):
            sample_dic[percent_li[i]] = []
        for i in range(len(percent_li)):

            for j in diff_sample_index_li[i]:
                sample_dic[percent_li[i]].append(holmes_id_li[j])
        with open("each_sample_rate_entity.json", "w") as fw:
                json.dump(sample_dic, fw, indent = 4)

        holmes_alias_dic = {}
        for holmes_id in sample_dic["3%"]:
            source_identifiers = json.loads(self.redis_conn.hget("holmes_Origin", holmes_id))["Meta"]["Identifier"]
            holmes_id_alias = []
            for source in source_identifiers:
                for alia in source_identifiers[source]:
                    holmes_id_alias.append(alia)
            holmes_alias_dic[holmes_id] = set(holmes_id_alias)
        
        holmes_ids = []
        alias_ids = []
        for key, value in holmes_alias_dic.items():
            holmes_ids.append(key)
            alias_ids.append(value)
        fifteen_per_entity_ids_dic = {"holmesid" : holmes_ids, "alias_ids": alias_ids}
        df = pd.DataFrame(fifteen_per_entity_ids_dic)
        df.to_csv('three_sammple_entityid.csv') 
