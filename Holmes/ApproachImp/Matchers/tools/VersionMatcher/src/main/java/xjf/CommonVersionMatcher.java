...
public class CommonVersionMatcher {
    ...
    public CommonVersionMatcher(String[] libraryFilePaths) throws IOException {
    ...
    /**
     * Set the generation of tmp documents (from version)
     * @param tmpDoc
     * @param version
     */
    protected void setTmpDocument(Document tmpDoc, String version) {
        String[] versions = version.split(" ");
        for (String v : versions) {
            tmpDoc.add(new TextField(FIELD_VERSION, version, Field.Store.YES));
        }
    }

    /**
     * Set the generation of documents (from version)
     * @param doc
     * @param version
     */
    protected void setDocument(Document doc, String version) {
        doc.add(new TextField(FIELD_VERSION, version, Field.Store.YES));
    }
     ...
     */
    protected Query setQuery(String version) throws ParseException {
        if (version != null && version.length() == 0) version= null;
        String queryString;
        queryString = String.format("(%s)^%f", version, 1.0);
        return new QueryParser(FIELD_VERSION, analyzer).parse(queryString);
    }
     ...
     * @throws ParseException
     */
    public final String[] search(String version) throws IOException, ParseException {
        version = preprocessLexical(version);

        Query query = this.setQuery(version);
        ...
        for (int i = 0; i < hits.length; ++i) {
            ...
            results[i] = hitDoc.get(FIELD_LANGUAGE) + DELIMITER + hitDoc.get(FIELD_COMPONENT) + DELIMITER + hitDoc.get(FIELD_VERSION) + DELIMITER + hit.score;
    ...
    private void build() throws IOException {
        ...
        for (String libraryFilePath : libraryFilePaths) {
            ...
            for (Object obj : array) {
                String versions = ((String) obj);
                String componentName = versions.substring(0, versions.indexOf(L1_SPLIT_STR));
                String version = versions.substring(versions.indexOf(L1_SPLIT_STR) + L1_SPLIT_STR.length(), versions.length());
                versions = version.replace(L1_SPLIT_STR, " ");
