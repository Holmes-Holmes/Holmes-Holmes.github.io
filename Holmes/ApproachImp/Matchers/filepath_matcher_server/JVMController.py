...
class FilepathMatcherJVMController:

    _MATCHER_JAR_PATH = config.TOOL_FILEPATH_MATCHER_PATH
    _JAVA_MAVEN_FILEPATH_LIST = config.JAVA_MAVEN_FILEPATH_PATH
    _JAVASCRIPT_NPM_FILEPATH_LIST = config.JS_NPM_FILEPATH_PATH
    _PYTHON_PYPI_FILEPATH_LIST = config.PYTHON_PYPI_FILEPATH_PATH
    _GO_FILEPATH_LIST = config.GO_FILEPATH_PATH
    ...
    def __enter__(self) -> 'FilepathMatcherJVMController':
        self.open()
        return self
    ...
    def get_common_matcher(self):
        if self._jvm_state and self._common_matcher is None:
            self._common_matcher = self._j_package.CommonFilepathMatcher(
                jpype.JArray(jpype.JString)(
                    [self._JAVA_MAVEN_FILEPATH_LIST, self._JAVASCRIPT_NPM_FILEPATH_LIST, self._PYTHON_PYPI_FILEPATH_LIST, self._GO_FILEPATH_LIST])
            )
        return self._common_matcher

    def get_java_matcher(self):
        if self._jvm_state and self._java_matcher is None:
            self._java_matcher = self._j_package.CommonFilepathMatcher(
                jpype.JArray(jpype.JString)([self._JAVA_MAVEN_FILEPATH_LIST])
            )
        return self._java_matcher

    def get_javascript_matcher(self):
        if self._jvm_state and self._javascript_matcher is None:
            self._javascript_matcher = self._j_package.CommonFilepathMatcher(
                jpype.JArray(jpype.JString)([self._JAVASCRIPT_NPM_FILEPATH_LIST])
            )
        return self._javascript_matcher

    def get_python_matcher(self):
        if self._jvm_state and self._python_matcher is None:
            self._python_matcher = self._j_package.CommonFilepathMatcher(
                jpype.JArray(jpype.JString)([self._PYTHON_PYPI_FILEPATH_LIST])
            )
        return self._python_matcher

    def get_go_matcher(self):
        if self._jvm_state and self._go_matcher is None:
            self._go_matcher = self._j_package.CommonFilepathMatcher(
                jpype.JArray(jpype.JString)([self._GO_FILEPATH_LIST])
            )
