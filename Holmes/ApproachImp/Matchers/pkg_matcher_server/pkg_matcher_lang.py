...
class LanguagePackageMatcher:

    ONE_VS_N = -1.
    _SPLIT_CH = '._-/:'

    def __init__(self, language: ProgLang, jvm_controller: PackageMatcherJVMController) -> None:
...
class JavaPackageMatcher(LanguagePackageMatcher):

    def __init__(self, jvm_controller: PackageMatcherJVMController) -> None:
    ...
    def search(self, cpe_vendor: Optional[str], cpe_product: Optional[str],
        ...
        for r in results:
            ...
            r_splt = r.split('#')
            r_lang = str(r_splt[0])
            r_name = str(r_splt[1]).replace("__fdse__", ":")
            r_score = str(r_splt[2])
...
class JavascriptPackageMatcher(LanguagePackageMatcher):
...
class PythonPackageMatcher(LanguagePackageMatcher):
...
class GoPackageMatcher(LanguagePackageMatcher):
...
def get_package_matcher(language: ProgLang, jvm_controller: PackageMatcherJVMController)\
        -> Optional[LanguagePackageMatcher]:
...
SUPPORTED_LANGUAGE_MATCHER = {
    ProgLang.JAVA: JavaPackageMatcher,
    ProgLang.JAVASCRIPT: JavascriptPackageMatcher,
    ProgLang.PYTHON: PythonPackageMatcher,
    ProgLang.GO: GoPackageMatcher
}
