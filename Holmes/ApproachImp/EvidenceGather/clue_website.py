...
def get_all_vuls_effective_refs():
    with open("./iterrate_process/inputclue_init.json", "r") as fr:
        ...
        r = redis.Redis(host='10.176.34.116', port=6379, db=0, password='Fudanse2022')
        for key in all_vul:
...
def crawl_core(deepvul_id, url_list, crawl_depth):
    ...
    driver = webdriver.Chrome(options=options, executable_path='/home/dellr740/dfs/data/Workspace/wss/Vulnerability_Database/exploring data analysis/component_ana/experiment/application_experiment/2.generalization_ability/driver/chromedriver_linux')
    # driver = webdriver.Chrome(options=options, executable_path='C:/Users/98082/workspace/Vulnerability_Database/exploring data analysis/component_ana/experiment/application_experiment/2.generalization_ability/evidencecolloctor/driver/chromedriver.exe')

    try:
        with open(f"./website_cache/{deepvul_id}/map_filename2url__fdse__{n}.json", "r") as fr:
    ...
    with open(f"./website_cache/{deepvul_id}/map_filename2url__fdse__{n}.json", "w") as fw:
...
def exploit_clue_extract(crawl_depth):
    ...
    for vul_id in current_refs:
        ...
        with open(f"./website_cache/{vul_id}/map_filename2url__fdse__{n}.json", "r") as fr:
